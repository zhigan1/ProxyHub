...
- 👥 **可视化多账号管理**：
  ...
  - **状态控制**：一键「启用 / 禁用」账号（禁用立即移出调度链），支持「彻底删除」物理文件与持久化黑名单；
  - **积分额度监控与修改**：实时显示账号可用积分，支持直接点击修改自定义额度，并提供「一键查询/刷新全部积分」实时同步。
- 🎁 **全平台自动签到与额度轮询**：
  - 借鉴开源项目 [cockpit-tools](https://github.com/jlcodes99/cockpit-tools) 理念，打通腾讯云代码助手与字节跳动 Trae 官方云端签到查额接口，全套模拟 CLI 请求头与自适应多层字段解析；
  - 网关启动与后台定时（默认 12 小时）自动完成全账号签到，同步刷新可用额度。
- ⚡ **分层自愈熔断机制**：
  - **模型级与账号级双层熔断**：针对 429 限流/额度耗尽、401/403 登录态失效自动触发账号级立即熔断，跨模型毫秒级自动切换到健康账号或备用平台；
  - `Closed` $\to$ `Open` $\to$ `Half-Open` 状态机，冷却期满放行单个请求探测自愈。
